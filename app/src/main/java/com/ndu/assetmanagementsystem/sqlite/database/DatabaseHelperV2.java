...
//    SELECT COUNT(*) FROM Asset WHERE asset_rfid IS NOT NULL and asset_status = 'Asset Ada';
    ...
    public int updateAsset(AssetV2 asset) {
        SQLiteDatabase db = this.getWritableDatabase();
        ...
        // updating row
        return db.update(TABLE_NAME_V2, values, /*COLUMN_ID*/COLUMN_TXTFIXEDASSETCODE + " = ?",
                /*new String[]{String.valueOf(asset.getId())});*/
                new String[]{asset.getTxtFixedAssetCode()});
    }

    public int updateStatusByRfid(AssetV2 asset, String rfid) {
        SQLiteDatabase db = this.getWritableDatabase();
